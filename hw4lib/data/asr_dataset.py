...
class ASRDataset(Dataset):
    def __init__(
        ...
        subset_size      = int(subset_raw * len(self.fbank_files)) if isinstance(subset_raw, float) else int(subset_raw)
        ...
        for i in tqdm(range(self.length)):
            ...
            if self.partition != "test-clean":
                ...
                transcript = np.load(self.text_files[i], allow_pickle=True).item()
    ...
    def collate_fn(self, batch) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        feats, shifted, golden = zip(*batch)
        batch_feats  = [f.T for f in feats]

        # TODO: Collect feature lengths from the batch into a tensor
        feat_lengths = torch.LongTensor([f.shape[0] for f in batch_feats]) # B
        ...
        if self.partition != "test-clean":
            ...
            transcript_lengths = torch.LongTensor([len(s) for s in shifted])
            padded_shifted     = pad_sequence(list(shifted), batch_first=True, padding_value=self.pad_token)
            padded_golden      = pad_sequence(list(golden),  batch_first=True, padding_value=self.pad_token)
